...
final class Store: ObservableObject {
    ...
    init() {
        ...
        fileURL = dir.appendingPathComponent("paniclog_entries.json")
        load()
    }
    ...
    func load() {
        if let data = try? Data(contentsOf: fileURL),
           let decoded = try? JSONDecoder().decode([EpisodeEntry].self, from: data) {
            ...
            entries = [
            EpisodeEntry(date: Date().addingTimeInterval(-0), title: "Before presentation", metric: 7, tag: "Breathing"),
            EpisodeEntry(date: Date().addingTimeInterval(-86400), title: "Crowded subway", metric: 8, tag: "Grounding")
            ]
            save()
        }
    }
